...
// ── PRAYER DATA ─────────────────────────────────────────────
const PRAYER_INFO = {
  Fajr:    { ar:'الفجر',   icon:'🌙', type:'Sunnah 2 + Fard 2',
  ...
  Dhuhr:   { ar:'الظهر',   icon:'☀️', type:'Sunnah 4 + Fard 4 + Sunnah 2',
  ...
  Asr:     { ar:'العصر',   icon:'🌤️', type:'Sunnah 4 + Fard 4',
  ...
  Maghrib: { ar:'المغرب',  icon:'🌅', type:'Fard 3 + Sunnah 2 + Nafl 2',
  ...
  Isha:    { ar:'العشاء',  icon:'🌙', type:'Sunnah 4 + Fard 4 + Sunnah 2 + Witr 3',
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  startCountdown();
  calcFitrana();
  updateRamadanInfo(); // NEW: show next Ramadan
    ...
    vList.innerHTML = `
      ...
      ${verses.map((v, i) => `
        <div class="verse-item" id="vi${i+1}">
          <div class="verse-arabic">${v.ar}
            <span style="font-size:.9rem;color:var(--teal-l);margin-right:8px">﴿${i+1}﴾</span>
          </div>
...
function renderAudioPlayer(surah) {
  ...
  ap.innerHTML = `
    <div class="audio-player">
      ...
      <select style="max-width:130px;font-size:.75px" id="reciterSelect"
...
function loadSurahAudio(num, reciter) {
  ...
  // Islamic Network CDN — reliable free source
  ...
  currentAudio.addEventListener('loadedmetadata', () => {
    ...
    if (durEl && currentAudio.duration) durEl.textContent = fmtTime(currentAudio.duration);
  });
  ...
  currentAudio.addEventListener('timeupdate', () => {
    ...
    if (timeEl) timeEl.textContent = fmtTime(currentAudio.currentTime);
  });
...
function calcZakat() {
  ...
  if (met && TOKEN) {
    apiFetch('/zakat/save', { method: 'POST', body: {
...
function startCountdown() {
  // Next Ramadan: ~Feb 28, 2026
  const nextRamadan = new Date('2026-02-28T00:00:00');
  ...
  function tick() {
    ...
    if (diff > 0) {
      // Show days instead of hours if far away
      if (days > 1) {
        ...
        if (labels[2]) labels[2].textContent = 'Mins';
        if (labels[3]) labels[3].textContent = 'To Ramadan';
      } else {
